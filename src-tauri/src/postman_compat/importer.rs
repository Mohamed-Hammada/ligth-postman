...
pub fn import_collection(
    ...
    let result = (|| -> Result<CollectionImportReport, AppError> {
        ...
        let collection_vars = collection.variable.iter().filter_map(|var| {
            let key = var.key.as_ref()?.trim().to_string();
            if key.is_empty() {
                return None;
            }
            let is_secret = var.r#type.as_deref() == Some("secret");
            Some((key, (var.value.clone().unwrap_or_default(), is_secret)))
        });
        for (key, (val, is_secret)) in super::dedupe_keys_last_wins(collection_vars) {
            // Only count what actually landed — the previous `let _ =` counted rejected
            // duplicates too, inflating the "variables imported" number in the report.
...
fn import_items_recursive(
    ...
    for item in items {
        ...
        if let Some(req) = &item.request {
            ...
            let mut headers: Vec<HeaderEntry> = req
                .header
                .as_deref()
                .unwrap_or(&[])
                .iter()
            ...
            if let Some(mime) = req.body.as_ref().and_then(infer_raw_content_type_header) {
...
/// Postman stores a raw body's format as `body.options.raw.language` (e.g. "json") rather than
/// a header — real collections routinely have zero explicit Content-Type header on a JSON
/// request and rely on the Postman app adding it at send-time. Without translating that hint
/// into an actual header on import, every raw body lands in the editor as bare "Text": no
/// syntax highlighting, and the Beautify button has nothing to go on until the user manually
/// picks a type. Only applies to "raw" mode — form-data/urlencoded/graphql set their own type.
fn infer_raw_content_type_header(body: &PostmanBody) -> Option<&'static str> {
    if body.mode.as_deref().unwrap_or("raw") != "raw" {
        return None;
    }
    let language = body
        .options
            ...
            if let Some(items) = &body.formdata {
                let parsed_items: Vec<FormDataPart> = items
                    ...
                    .map(|i| {
                        ...
                        let file_path = i.src.as_deref().map(super::normalize_local_file_src);
                        if is_file {
                            let still_here = file_path.as_deref().is_some_and(|p| std::path::Path::new(p).is_file());
                            if !still_here {
                                warnings.push(format!(
                                    "Request '{req_name}': multipart file part '{}' references local file '{}' — re-select it on this machine if the path doesn't exist here.",
                                    i.key,
                                    file_path.as_deref().unwrap_or("<empty>")
                                ));
                            }
                        }
                        FormDataPart {
...
pub fn import_environment(
    ...
    let env_vars = env.values.iter().filter_map(|item| {
        let key = item.key.trim().to_string();
        if key.is_empty() {
            return None;
        }
        let is_secret = item.r#type.as_deref() == Some("secret");
        Some((key, (item.value.clone(), item.enabled, is_secret)))
    });
    for (key, (value, enabled, is_secret)) in super::dedupe_keys_last_wins(env_vars) {
        if variable_store::create_variable(
            conn,
            NewVariableInput {
