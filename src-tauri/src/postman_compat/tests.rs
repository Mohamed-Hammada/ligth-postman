...
/// Postman normalizes a multipart file part's local path to `/C:/Users/...` on Windows — a
/// POSIX-style leading slash in front of the drive letter that Windows file APIs reject outright,
/// so even a file that's still sitting right there on disk read as unopenable. And when the file
/// really is gone, the warning should say so plainly rather than always claiming "re-select it"
/// regardless of whether that's actually necessary.
#[test]
fn normalizes_windows_drive_letter_file_src_and_skips_warning_when_file_still_exists() {
    let conn = setup_db();
    let still_here = std::env::temp_dir().join(format!("postman-import-test-{}.png", uuid::Uuid::new_v4()));
    std::fs::write(&still_here, b"fake image bytes").unwrap();
    let still_here_src = format!("/{}", still_here.to_string_lossy().replace('\\', "/"));

    let json = format!(
        r#"{{
        "info": {{ "name": "File Paths Test" }},
        "item": [
            {{
                "name": "Still There",
                "request": {{
                    ...
                    "body": {{ "mode": "formdata", "formdata": [
                        {{ "key": "avatar", "src": {still_here_src:?}, "type": "file" }}
                    ] }}
                }}
            }},
            {{
                ...
                "request": {{
                    ...
                    "url": "https://api.test.com/upload",
                    "body": {{ "mode": "formdata", "formdata": [
                        {{ "key": "avatar", "src": "/C:/Users/nobody/Downloads/deleted.png", "type": "file" }}
                    ] }}
                }}
            }}
        ]
    ...
    let report = import_collection(&conn, &json, None, "default").unwrap();
    let _ = std::fs::remove_file(&still_here);

    let reqs = request_store::list_requests(&conn, &report.project_id).unwrap();
    let resolve = |t: &str| t.to_string();

    let still_here_req = reqs.iter().find(|r| r.name == "Still There").unwrap();
    let still_here_req = request_store::get_request(&conn, &still_here_req.id).unwrap();
    let (_, multipart, _, _) = crate::canonical_request::resolve_body(still_here_req.body.as_deref(), &resolve);
    let parts = multipart.unwrap();
    assert!(parts.iter().any(|p| matches!(
        p,
        crate::models::ResolvedFormPart::File { key, file_path, .. }
        if key == "avatar" && file_path == &still_here.to_string_lossy()
    )));

    let long_gone_req = reqs.iter().find(|r| r.name == "Long Gone").unwrap();
    let long_gone_req = request_store::get_request(&conn, &long_gone_req.id).unwrap();
    let (_, multipart, _, _) = crate::canonical_request::resolve_body(long_gone_req.body.as_deref(), &resolve);
    let parts = multipart.unwrap();
    assert!(parts.iter().any(|p| matches!(
        p,
        crate::models::ResolvedFormPart::File { key, file_path, .. }
        if key == "avatar" && file_path == r"C:\Users\nobody\Downloads\deleted.png"
    )));

    assert!(
        !report.warnings.iter().any(|w| w.contains("Still There")),
        "a file that still exists at the normalized path shouldn't warn: {:?}",
        report.warnings
    );
    assert!(
        report.warnings.iter().any(|w| w.contains("Long Gone")),
        "a genuinely missing file should still warn: {:?}",
        report.warnings
    );
}

/// Real Postman collections routinely have no explicit Content-Type header on a raw JSON body —
/// Postman infers it client-side from `body.options.raw.language` and adds it only at send-time,
/// so it's rarely present in the exported file. Without translating that hint into a real header
/// on import, the request editor's raw-type dropdown falls back to "Text" and the Beautify
/// button has nothing to detect from.
#[test]
fn imports_raw_json_body_language_hint_as_content_type_header() {
    let conn = setup_db();
    let json = r#"{
        "info": { "name": "Raw Body Language Test" },
        "item": [
            {
                ...
                "request": {
                    ...
                    "body": {
                        "mode": "raw",
                        "raw": "{\"approved_ids\": [\"123\"]}",
                        "options": { "raw": { "language": "json" } }
                    }
                }
            },
            {
                "name": "Already Has Content-Type Header",
                "request": {
                    "method": "POST",
                    "url": "https://api.test.com/status",
                    "header": [
                        { "key": "Content-Type", "value": "application/vnd.custom+json" }
                    ],
                    "body": {
                        "mode": "raw",
                        "raw": "{\"x\": 1}",
    ...
    let no_header_req = reqs.iter().find(|r| r.name == "No Content-Type Header").unwrap();
    let no_header_req = request_store::get_request(&conn, &no_header_req.id).unwrap();
    assert_eq!(no_header_req.headers.len(), 1);
    assert_eq!(no_header_req.headers[0].key, "Content-Type");
    assert_eq!(no_header_req.headers[0].value, "application/json");

    // An explicit header already in the collection must never be overridden by the inferred one.
    let has_header_req = reqs.iter().find(|r| r.name == "Already Has Content-Type Header").unwrap();
    let has_header_req = request_store::get_request(&conn, &has_header_req.id).unwrap();
    assert_eq!(has_header_req.headers.len(), 1);
    assert_eq!(has_header_req.headers[0].value, "application/vnd.custom+json");
}
