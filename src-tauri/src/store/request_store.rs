...
/// Workspace-wide search for the command palette (LP-1404) and the sidebar's own APIs/All scope
/// — matches across every project in the workspace, not just whichever one happens to be open.
/// `fields` picks which columns count as a match (any of "name", "url", "body"; unrecognized
/// values are ignored) — the caller drives this from its own Name/URL/Body toggle. An empty or
/// all-unrecognized list matches nothing rather than falling back to "everything", so unchecking
/// every field in the UI reads as "no fields to search in" instead of silently ignoring the
/// filter. Body is only searchable here, never in the frontend's own in-project request list —
/// that list deliberately never loads bodies (they stay on disk until a tab opens them — README
/// §4/§20), so body search only exists through this direct-SQL path. A leading `%` wildcard means
/// this can't use an index, but that's fine at the scale a local single-user SQLite file actually
/// holds; move to FTS5 only if that stops being true.
pub fn search_requests_in_workspace(
    ...
    if fields.iter().any(|f| f == "name") {
        clauses.push("r.name LIKE ?2 ESCAPE '\\'");
    }
    if fields.iter().any(|f| f == "url") {
        clauses.push("r.url LIKE ?2 ESCAPE '\\'");
    }
    if fields.iter().any(|f| f == "body") {
        clauses.push("r.body LIKE ?2 ESCAPE '\\'");
    }
    if clauses.is_empty() {
        return Ok(Vec::new());
    }
...
mod tests {
    ...
    fn seed_project(conn: &Connection) -> String {
        project_store::create_project(conn, NewProjectInput { name: "Demo".into(), workspace_id: "default".into() })
            .unwrap()
            .id
    }
    ...
    fn search_in_workspace_finds_matches_across_every_project_in_it() {
        ...
        let name_and_url = vec!["name".to_string(), "url".to_string()];

        // Matches by name, across both projects in the "default" workspace.
        let results =
            search_requests_in_workspace(&conn, "default", "refund", &name_and_url, 30).unwrap();
        ...
        // Matches by URL too.
        let by_url =
            search_requests_in_workspace(&conn, "default", "users", &name_and_url, 30).unwrap();
        assert_eq!(by_url.len(), 1);
        assert_eq!(by_url[0].project_id, project_a);

        // With only "name" selected, a URL-only match is excluded.
        create_request(
            &conn,
            NewRequestInput {
                project_id: project_a.clone(),
                name: "Ping".into(),
                method: "GET".into(),
                url: "https://api.example.com/status-check-9001".into(),
                ..Default::default()
            },
        )
        .unwrap();
        let name_only = vec!["name".to_string()];
        let url_excluded = search_requests_in_workspace(
            &conn,
            "default",
            "status-check-9001",
            &name_only,
        ...
        .unwrap();
        assert_eq!(url_excluded.len(), 0);

        // Body is only searched when explicitly requested, and never through name/url.
        create_request(
            &conn,
            NewRequestInput {
                project_id: project_a.clone(),
                ...
                body: Some(r#"{"sku":"widget-alpha-9000"}"#.into()),
                ..Default::default()
            },
        )
        .unwrap();
        let body_not_searched_by_default = search_requests_in_workspace(
            &conn,
            "default",
            "widget-alpha-9000",
            &name_and_url,
        ...
        .unwrap();
        assert_eq!(body_not_searched_by_default.len(), 0);
        let by_body = search_requests_in_workspace(
            &conn,
            "default",
            "widget-alpha-9000",
            &["body".to_string()],
        ...
        assert_eq!(by_body[0].project_id, project_a);

        // No fields selected means no matches, not "match everything".
        let no_fields = search_requests_in_workspace(&conn, "default", "refund", &[], 30).unwrap();
        assert_eq!(no_fields.len(), 0);
    }
