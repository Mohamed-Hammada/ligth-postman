...
/// Backs the command palette's and sidebar's workspace-wide search (LP-1404) — every project in
/// the workspace, not just whichever one is currently open. `fields` is the caller's Name/URL/Body
/// toggle state, forwarded as-is to `request_store::search_requests_in_workspace`.
#[tauri::command]
pub fn search_requests_in_workspace(
    ...
    let conn = state.db.lock().expect("db mutex poisoned");
    request_store::search_requests_in_workspace(&conn, &workspace_id, &query, &fields, 30)
}
