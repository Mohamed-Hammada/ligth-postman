...
// Outside the Tauri shell (plain `vite dev` in a browser) calls are served by an in-memory mock.
// It lives in ./dev-mock and is imported dynamically behind `import.meta.env.DEV`, so it (and the
// git-ignored SQLite snapshot it reads) can never end up in, or slow down, a release build.
async function callDevMock<T>(cmd: string, args?: Record<string, unknown>): Promise<T> {
  if (!import.meta.env.DEV) {
    throw new Error("This runs as a desktop app — open it via the app window, not a browser tab.");
  }
  const mock = await import("./dev-mock");
  return mock.invokeDevMock<T>(cmd, args);
}

/** Same call shape as the real `invoke`, but checked up front — every one of this file's ~80
 * call sites goes through here. Outside the Tauri shell it serves the dev mock (so the UI can be
 * previewed in a plain browser). In the Tauri runtime it calls tauriInvoke. */
function invoke<T>(cmd: string, args?: Record<string, unknown>): Promise<T> {
  if (!isTauriRuntime()) return callDevMock<T>(cmd, args);
