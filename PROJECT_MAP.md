...
| Dynamic variables (§LP-0210) | `resolver::dynamic_value` — `{{$guid}}`, `{{$timestamp}}`, `{{$isoTimestamp}}`, `{{$randomInt}}`, generated fresh on every resolution, never persisted; only used as a fallback when no scope-map entry exists, so a real variable literally named e.g. `$guid` still wins; an unrecognized `$name` still reports as missing | 6 `resolver::tests::*`: valid-UUID-and-changes-every-call, numeric timestamp, valid RFC3339, bounded random int, persisted-variable-takes-precedence, unrecognized-token-stays-missing |
| CanonicalRequest (§LP-0303) | `canonical_request.rs::build(request, chain)` — the one fully-resolved request representation (method/url/headers/body, auth folded in) shared by `execution.rs` (sending) and `codegen.rs` (snippets). Pure, DB-free, like `resolver.rs`/`http_engine.rs`. `execution.rs`'s old inline resolution logic was deleted, not left as dead code | 6 `canonical_request::tests::*` (auth variants, disabled-field dropping, body resolution) |
| Authorization model (§LP-0107) | `models::Auth` — None/Bearer/Basic/ApiKey (header or query location); stored as its own migrated column (migration 6); every field resolved through the same `{{var}}` chain as headers/body; applied to the CanonicalRequest exactly once. "Inherit"/OAuth2 explicitly not built — no Collection to inherit auth from, no OAuth2 flow; a fake `Inherit` variant would be exactly the placeholder behavior this pack forbids | `canonical_request::tests::*` (bearer/basic/api-key-header/api-key-query/none), `request_store::tests::update_can_change_auth_type`, and the extended `execution::tests::full_pipeline_...` — a real network request with a genuinely resolved `Authorization: Bearer secret-<port>` header on the wire |
| Code snippet generation (§LP-0601/0602/0605/0606) | `codegen.rs` — `generate_snippet` always builds a `CanonicalRequest` first (never reads raw UI text); `generate_curl_bash` with correct POSIX single-quote escaping, `-H`/`--data-raw`; `SnippetMode::Placeholder` (default, resolves nothing) vs `Resolved` (explicit opt-in) | 5 `codegen::tests::*` incl. an exact-string structural check and embedded-quote escaping; frontend Code Snippet panel (mode select, Generate, Copy to clipboard) verified to type-check and build |
...
**Manual smoke tests:** (1) fresh DB — migrations 1-4 applied, ~29 MB idle RSS; (2) pre-existing DB from a prior run at migration 3 — only migration 4 applied on top; (3) after adding `reqwest`/`tokio` — app still launches, idle RSS ~31.5 MB (small, expected increase from the TLS/async stack, still light); (4) after migration 5 (query params) — applied cleanly on top of an existing DB at migration 4; (5) after adding the AI provider — app launches, logs "AI provider configured: false" (no `ANTHROPIC_API_KEY` in this environment), no crash; (6) after migration 6 (auth column) — applied cleanly on top of an existing DB at migration 5.
...
- Auth has no `Inherit` or OAuth2 variant — there's no Collection/Folder to inherit from yet, and no OAuth2 flow. Every request's auth is set directly on that request.
- cURL snippet generation has no `Masked` mode (only `Placeholder` and `Resolved`) — masking would need the resolver to track which output bytes came from a secret variable, which it doesn't do today. Only Bash is generated; PowerShell/CMD/other languages and the cURL importer (LP-0603/0604/0607/0608/0609/0610) aren't built.
- HTTP Basic auth is base64-encoded correctly, but nothing prevents a user from putting real credentials directly in the Basic auth fields (as opposed to a `{{variable}}`) — same general secrets-hygiene caveat as everywhere else in this app; there is no separate warning for it.
...
- **46 DONE** — implemented, tested, and either runtime- or real-HTTP-verified.
...
- **4 BLOCKED** — the task text names a specific prerequisite that doesn't exist: `LP-0902` blocks on `LP-0901` (script sandbox); `LP-0817`/`LP-0818` block on the source-analyzer pipeline (`LP-0812`-`0815`); `LP-0913` (Linux packaging) blocks on this session having no Linux build environment. Phase 06 (cURL/codegen) is **no longer blocked** — `LP-0303` (`CanonicalRequest`) now exists.
- **83 TODO** — not started, no blocker.
...
- [x] ~~Authorization model~~ — done (see DONE table). LP-0107.
- [ ] Body variants (form/multipart/GraphQL), cookie jar, request settings (timeout/redirect/proxy/TLS), request docs field — task-pack Phase 01 remainder (LP-0108-0115).
- [ ] cURL PowerShell/CMD generators, additional-language generators, cURL importer — task-pack Phase 06 remainder (LP-0603/0604/0607/0608/0609/0610), no longer blocked now that LP-0303 (CanonicalRequest) exists.
